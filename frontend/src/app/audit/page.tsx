...
import { AlertTriangle, CalendarRange, CheckCircle2, ChevronLeft, ChevronRight, Clock3, Copy, Eye, Siren, RefreshCw, Route, Search, ShieldAlert, ShieldCheck, UserSearch, Waypoints, X } from "lucide-react";
...
export default function AuditPage() {
	...
	const applyQuickFocus = async (focus: "failed" | "routing" | "incidents") => {
		const nextFilters = focus === "failed" ? { ...filters, status: "FAILED", resourceType: "ALL" } : focus === "routing" ? { ...filters, resourceType: "ROUTING_RUN", status: "ALL" } : { ...filters, status: "FAILED", resourceType: "ROUTING_RUN" };
	...
	return (
		<ProtectedRoute>
							...
							<div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
								<div className="space-y-3">
									...
									<div>
										...
										<p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground md:text-base">Theo dõi thao tác nhạy cảm, đối chiếu dữ liệu trước và sau, và truy vết request giúp admin điều tra nhanh hơn.</p>
									</div>
								</div>

								<div className="grid gap-3 sm:grid-cols-2 xl:w-[460px]">
									<SummaryCard icon={Waypoints} label="Tổng kết quả" value={String(totalElements)} helper={resultSummary} tone="neutral" />
									<SummaryCard icon={AlertTriangle} label="Bản ghi thất bại" value={String(currentPageFailedCount)} helper="Tính trên trang hiện tại" tone="danger" />
								</div>
							</div>
						</div>
					</div>
						...
						{canReadAudit ? (
							<>
								<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
									<SummaryCard icon={Clock3} label="Trang hiện tại" value={String(logs.length)} helper="Số dòng đang hiển thị" tone="neutral" />
									<SummaryCard icon={CheckCircle2} label="Thành công" value={String(currentPageSuccessCount)} helper="Tác vụ thành công" tone="success" />
									<SummaryCard icon={AlertTriangle} label="Thất bại" value={String(currentPageFailedCount)} helper="Cần ưu tiên kiểm tra" tone="danger" />
									<SummaryCard icon={Route} label="Nhật ký routing" value={String(currentPageRoutingCount)} helper="Sự kiện tối ưu tuyến" tone="routing" />
								</div>

								<div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr_0.9fr]">
									<PrioritySpotlightCard icon={Siren} title="Cảnh báo ưu tiên" value={`${currentPageIncidentCount} bản ghi`} description="Gom các sự kiện thất bại và routing để admin quét trước." tone="danger" onClick={() => void applyQuickFocus("incidents")} buttonLabel="Tập trung sự cố" />
									...
									/>
									<PrioritySpotlightCard icon={Route} title="Tối ưu tuyến" value={`${currentPageRoutingCount} bản ghi`} description="Nhóm log tối ưu tuyến, để theo dõi run thành công và các ca fail." tone="routing" onClick={() => void applyQuickFocus("routing")} buttonLabel="Chỉ xem routing" />
								</div>
									...
									<div className="space-y-5 p-6">
										<div className="grid gap-4 lg:grid-cols-3 xl:grid-cols-6">
											<div className="xl:col-span-2">
												...
												<Input id="audit-search" className="mt-2" placeholder="Tên người dùng, tài nguyên, thông điệp, mã yêu cầu..." value={filters.search} onChange={(event) => setFilters((current) => ({ ...current, search: event.target.value }))} />
											</div>
											<div>
											...
											<div>
												...
												<Input id="audit-actor" className="mt-2" placeholder="Nhập username" value={filters.actorUsername} onChange={(event) => setFilters((current) => ({ ...current, actorUsername: event.target.value }))} />
											</div>
											<div>
										...
										<div className="grid gap-4 md:grid-cols-2">
											<div>
												...
												<Input id="audit-from" type="datetime-local" className="mt-2" value={filters.from} onChange={(event) => setFilters((current) => ({ ...current, from: event.target.value }))} />
											</div>
											<div>
												<Label htmlFor="audit-to">Đến thời điểm</Label>
												<Input id="audit-to" type="datetime-local" className="mt-2" value={filters.to} onChange={(event) => setFilters((current) => ({ ...current, to: event.target.value }))} />
											</div>
										</div>
												...
												{logs.length > 0 ? (
													logs.map((log) => (
														...
														>
															...
															<p className="mt-3 line-clamp-2 text-sm leading-6 text-muted-foreground">{log.message || "Không có thông điệp bổ sung cho bản ghi này."}</p>
													...
													<div className="rounded-2xl border border-dashed border-border px-6 py-16 text-center text-sm text-muted-foreground">{isLoading ? "Đang tải dữ liệu..." : "Không có bản ghi audit phù hợp."}</div>
												)}
											</div>
										</div>
												...
												<div className="space-y-6">
													...
													<div className="rounded-2xl border border-border bg-muted/20 p-5">
														<div className="flex items-center justify-between gap-3">
															...
															<span className="rounded-full border border-border bg-background px-3 py-1 text-xs font-medium text-muted-foreground">{highlightedChanges.length} mục</span>
														</div>
													...
													<div className="grid gap-4 xl:grid-cols-3">
														<AuditJsonBlock title="Dữ liệu trước" value={selectedLog.beforeData} onOpen={() => setIsJsonViewerOpen(true)} />
														<AuditJsonBlock title="Dữ liệu sau" value={selectedLog.afterData} onOpen={() => setIsJsonViewerOpen(true)} />
														<AuditJsonBlock title="Siêu dữ liệu" value={selectedLog.metadata} onOpen={() => setIsJsonViewerOpen(true)} />
													</div>
												</div>
											) : (
												<div className="rounded-2xl border border-dashed border-border px-6 py-16 text-center">
													<p className="text-lg font-semibold text-foreground">Chọn một bản ghi audit</p>
													<p className="mt-2 text-sm text-muted-foreground">Panel này sẽ hiện request context, snapshot dữ liệu và tóm tắt thay đổi.</p>
												</div>
											)}
										</div>
									</Card>
			...
			{selectedLog && isJsonViewerOpen ? <AuditJsonViewerModal log={selectedLog} onClose={() => setIsJsonViewerOpen(false)} /> : null}
		</ProtectedRoute>
	);
}

function SummaryCard({ icon: Icon, label, value, helper, tone }: { icon: typeof ShieldCheck; label: string; value: string; helper: string; tone: "neutral" | "success" | "danger" | "routing" }) {
	return (
		<Card className={cn("border-border/70 p-4 shadow-sm", tone === "success" && "border-emerald-500/20 bg-emerald-500/5", tone === "danger" && "border-rose-500/20 bg-rose-500/5", tone === "routing" && "border-sky-500/20 bg-sky-500/5")}>
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm">
			<div className="flex max-h-[90vh] w-full max-w-7xl flex-col overflow-hidden rounded-3xl border border-white/10 bg-slate-950 shadow-2xl">
				...
				<div className="grid flex-1 gap-4 overflow-auto p-6 xl:grid-cols-3">
					...
					<AuditJsonPanel title="Siêu dữ liệu" value={log.metadata} />
				</div>
			</div>
		</div>
	);
...
function countActiveFilters(filters: Filters) {
	return [filters.search, filters.action !== "ALL" ? filters.action : "", filters.resourceType !== "ALL" ? filters.resourceType : "", filters.actorUsername, filters.scopeDepotId !== "ALL" ? filters.scopeDepotId : "", filters.status !== "ALL" ? filters.status : "", filters.from, filters.to].filter(
		Boolean,
	).length;
}
