...
import { Trash2, Edit2, Package, Loader2 } from "lucide-react";
				...
				<tbody className="divide-y divide-border">
					{orders.map((order) => {
						const getStatusConfig = (status: OrderStatus) => {
							...
							if (status === OrderStatus.ASSIGNED) return { label: "Đã phân công", color: "bg-yellow-500/10 text-yellow-600 border-yellow-500/20" };
